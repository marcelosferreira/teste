...
import front_masks
from front_validations import validate_latitude, validate_longitude, CoordenadasInvalidasError
#import folium COMENTADO A RETIRAR
import webbrowser    
import json
...
def criar_etapa3(self,conteudo,etapa3):
        ...
        #AQUI
        titulos = ["Seleção de usinas para a simulação", "Nome da usina", "Selecione uma usina cadastrada", "Fonte de energia", "Latitude", "Longitude", " ", "Potência [MW]", "Capacidade do reservatório em caso de PCH [h]", "Aumentar fator de capacidade da geração eólica", "IP: Taxa de paradas programadas [%]", "IP: Taxa de paradas não programadas [%]", "Taxa de risco de curtailment [%]", "Taxa de degradação [%/ano]", "Taxa de depreciação contábil (%)", "CAPEX [$]", "OPEX [$/kW*ano]", "Pagamento de TUSTg [$/kW]", "Geração no mesmo local do consumo", "Idade da usina [anos]", "Previsão da geração [kWh/h]", "Caminho do Arquivo           ", "Estimar LCOE da usina ou deseja inserir projeção de preço/custo da usina?", "Custo/Preço de Energia [$/kWh]", "Caminho do Arquivo           "]
        #ATE AQUI
        ...
        for i in range(20):
            ...
            capacidade_reservatorio_entry = ttk.Entry(conteudo, textvariable=capacidade_reservatorio_var)
            ...
            ##DAQUI
            salvar_button = ttk.Button(conteudo, text="Salvar Usina", command=lambda i=i: salvar_linha(i))
            salvar_button.grid(row=i+1, column=25, padx=10, pady=5)
        
        #DAQUI        
        def carregar_opcoes_cadastradas():
            try:
                with open("dados_usinas.json", "r") as arquivo_json:
                    dados_usinas = json.load(arquivo_json)
            except FileNotFoundError:
                dados_usinas = []
            opcoes_usinas = [usina["Nome da usina"] for usina in dados_usinas]
            for i in range(20):
                self.menu_usinas = ttk.Combobox(conteudo, values=opcoes_usinas, validate="all", validatecommand=(conteudo.register(lambda P: self.validar_combobox_input(P, opcoes_usinas)), '%P'))
                self.menu_usinas.grid(row=i+1, column=3, padx=10, pady=5, sticky="w")
            return [usina["Nome da usina"] for usina in dados_usinas]
        
        carregar_opcoes_cadastradas()

        def salvar_linha(i):
            linha_data = {
                "Nome da usina": self.nome_usina_entries[i].get(),
                "Selecionar": self.selecionar_entries[i].get(),
                "Fonte de energia": self.fonte_energia_entries[i].get(),
                "Latitude": self.lat_entries[i].get(),
                "Longitude": self.long_entries[i].get(),
                "Potência [MW]": self.potencia_entries[i].get(),
                "Capacidade do reservatório em caso de PCH [h]": self.capacidade_reservatorio_entries[i].get(),
                # Adicione outros campos conforme necessário
            }

            # Caminho para o arquivo JSON onde os dados serão salvos
            caminho_arquivo_json = "dados_usinas.json"
            ...
            # Adicione os dados da linha ao JSON existente
            dados_existente.append(linha_data)
        ...
        def defaultEnergia():
                ...
                self.nome_usina_entries[0].set("Eólica 1")
                self.fonte_energia_entries[0].set("Eólica")
                self.lat_entries[0].set("-5.127838")
                self.long_entries[0].set("-36.453683")
                self.potencia_entries[0].set("68.47")
                ...
                self.tx_risco_curtailment_entries[0].set("1")
                self.tx_degradacao_entries[0].set("1")
                self.tx_depreciacao_contabil_entries[0].set("10")
                self.capex_fontes_entries[0].set("102000000")
                self.opex_fontes_entries[0].set("5135250")
                self.pagamento_tustg_entries[0].set("7")
                self.idade_usina_entries[0].set("0")
                self.proj_preco_custo_entries[0].set("0")
                self.geracao_local_entries[0].set(1)

                self.nome_usina_entries[1].set("Eólica 2")
                self.fonte_energia_entries[1].set("Eólica")
                self.lat_entries[1].set("-5.127838")
                self.long_entries[1].set("-36.453683")
                self.potencia_entries[1].set("60.12")
                self.capacidade_reservatorio_entries[1].set("0")
                self.ip_paradas_prog_entries[1].set("2.5")
                self.ip_paradas_nao_prog_entries[1].set("2.5")
                self.tx_risco_curtailment_entries[1].set("1")
                self.tx_degradacao_entries[1].set("1")
                self.tx_depreciacao_contabil_entries[1].set("10")
                self.capex_fontes_entries[1].set("90000000")
                self.opex_fontes_entries[1].set("4509000")
                self.pagamento_tustg_entries[1].set("7")
                self.idade_usina_entries[1].set("0")
                self.proj_preco_custo_entries[1].set("0")
                self.geracao_local_entries[1].set(1)

                self.nome_usina_entries[2].set("Solar 1")
                self.fonte_energia_entries[2].set("FV")
                self.lat_entries[2].set("-5.127838")
                self.long_entries[2].set("-36.453683")
                self.potencia_entries[2].set("68.47")
                self.capacidade_reservatorio_entries[2].set("0")
                self.ip_paradas_prog_entries[2].set("2.5")
                self.ip_paradas_nao_prog_entries[2].set("2.5")
                self.tx_risco_curtailment_entries[2].set("1")
                self.tx_degradacao_entries[2].set("1")
                self.tx_depreciacao_contabil_entries[2].set("10")
                self.capex_fontes_entries[2].set("44200000")
                self.opex_fontes_entries[2].set("3081150")
                self.pagamento_tustg_entries[2].set("7")
                self.idade_usina_entries[2].set("0")
                self.proj_preco_custo_entries[2].set("0")
                self.geracao_local_entries[2].set(1)
        ...
        def masklat(event, entry, var):
            lat = var.get()
            lat_mask = front_masks.mask_coordenadas(lat)
            entry.delete(0, tk.END)
            entry.insert(0, lat_mask)
    
        def maskDec(event, entry, var):
            c = var.get()
            c_mask = front_masks.mask_decimal(c)
            entry.delete(0, tk.END)
        ...
        def atualizar_e_abrir_mapa(i):
            try:
                ...
                #mapa = folium.Map(location=[latitude, longitude], zoom_start=6) COMENTADO A RETIRAR

                #folium.Marker([latitude, longitude], tooltip="Localização").add_to(mapa) COMENTADO A RETIRAR
                #mapa.save('mapa.html') COMENTADO A RETIRAR
